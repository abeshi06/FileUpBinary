...
class ColorsControllerTest < ActionController::TestCase
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post :create, color: { colorname: @color.colorname, image: @color.image, username: @color.username }
    end
  ...
  test "should update color" do
    patch :update, id: @color, color: { colorname: @color.colorname, image: @color.image, username: @color.username }
    assert_redirected_to color_path(assigns(:color))
  end
